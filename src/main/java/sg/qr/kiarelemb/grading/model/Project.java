...
private static final Pattern SUBJECTIVE_NUMBER_LINE = Pattern.compile("^(\\d+)\\s*(?:[.\\uFF0E\\u3001:\\uFF1A])?\\s*(.*)$");
	...
	public Project(File projectFilePath) {
		this.projectFilePath = projectFilePath.getAbsolutePath();
	}

	public Project(String projectFilePath, String templateFilePath, String answerDirectoryPath, String[] standardAnswers, List<File> answerFiles) {
	...
	public Project(String projectFilePath, String templateFilePath, String answerDirectoryPath, String[] standardAnswers,
	...
	public void write() {
		...
		list.add("# recognized.<鍑嗚€冭瘉鍙?=璇嗗埆鐨勭瓟妗堬紝鐢ㄧ┖鏍煎垎寮€");
	...
	private List<File> answerImagesFromDirectory(File directory) throws java.io.IOException {
		if (convertedPdfFileName != null && !convertedPdfFileName.isBlank()) {
			...
			List<File> convertedImages = DocumentImageLoader.convertedPdfImages(convertedPdf);
			if (convertedImages.size() == convertedPdfImageCount && convertedPdfImageCount > 0) {
				return convertedImages;
			}
		}
		List<File> images = DocumentImageLoader.sortedAnswerImages(directory);
		File singlePdf = DocumentImageLoader.singlePdfFile(directory);
