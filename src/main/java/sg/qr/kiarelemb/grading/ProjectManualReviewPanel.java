...
public final class ProjectManualReviewPanel extends QRPanel implements ProjectStateSaver {
	...
	public ProjectManualReviewPanel(Project project, Template template) {
	...
	public static boolean hasManualQuestions(Template template) {
		return !manualRegions(template).isEmpty();
	}

	public static boolean allManualScoresSaved(Project project, Template template) {
		if (project == null || project.answerFiles() == null || project.answerFiles().isEmpty()) {
			return true;
		}
		List<SubjectiveRegion> regions = manualRegions(template);
		if (regions.isEmpty()) {
			return true;
		}
	...
	private BufferedImage crop(BufferedImage image, SubjectiveRegion region) {
	...
	private void backToSubjectiveReview() {
		saveCurrent();
		if (ProjectSubjectiveReviewPanel.hasSubjectiveQuestions(project, template)) {
			MainWindow.INSTANCE.setCenterComponent(new ProjectSubjectiveReviewPanel(project, template));
		} else {
	...
	private static List<ManualReviewItem> buildItems(Project project, Template template) {
	...
	private static List<SubjectiveRegion> manualRegions(Template template) {
		if (template == null || template.subjectiveRegions() == null) {
			return List.of();
		}
		return template.subjectiveRegions().stream()
				.filter(region -> region.mode() == SubjectiveRegion.GradingMode.MANUAL
								  || region.mode() == SubjectiveRegion.GradingMode.MIXED)
				.toList();
	}

	private static String savedScore(Map<String, String> savedScores, SubjectiveRegion region) {
	...
	public static String scoreForRule(Map<String, String> savedScores, String ruleName, Collection<Integer> questionNumbers, Template template) {
		...
		for (SubjectiveRegion region : manualRegions(template)) {
	...
	private static boolean sameManualSection(String sectionName, SubjectiveRegion region) {
		return sectionName.equals(region.name());
	}

	private static boolean covers(SubjectiveRegion region, Collection<Integer> questionNumbers) {
	...
	private record ManualReviewItem(File answerFile, File imageFile, String examineeId, SubjectiveRegion region) {
	}
}
