...
import sg.qr.kiarelemb.grading.end.ScoreRule;
 ...
 * @className ScaleScoreConfigDialog
 * @description TODO
 * @create 2026/6/10 16:13
 */
public class ScaleScoreConfigDialog extends QRDialog {
	private final ProjectEnd projectEnd;
	...
	private QuestionScoring.ScaleScoreReport report;
	private QuestionScoring.Config config;
	private boolean confirmed;

	public ScaleScoreConfigDialog(ProjectEnd projectEnd, QuestionScoring.Config config) {
	...
	private void addConfigRow(QRPanel panel, GridBagConstraints gbc, String label, Component field) {
		...
		qrLabel.setPreferredSize(new Dimension(95, ProjectEnd.FIELD_HEIGHT));
		panel.add(qrLabel, gbc);
		gbc.gridx = 1;
		gbc.weightx = 1;
		field.setPreferredSize(new Dimension(120, ProjectEnd.FIELD_HEIGHT));
		panel.add(field, gbc);
		gbc.gridy++;
	}
	...
	private void fillConfig(QuestionScoring.Config config) {
	...
	private boolean refreshPreview() {
		try {
			...
			return true;
		} catch (ProjectEnd.ScoreRuleException ex) {
	...
	private QuestionScoring.Config readConfig() {
		return new QuestionScoring.Config(
				QuestionScoring.WeightFunction.valueOf(String.valueOf(weightFunctionBox.getSelectedItem())),
	...
	private void renderScalePreview(QuestionScoring.ScaleScoreReport report) {
		previewModel.setRowCount(0);
		Map<Integer, String> sectionByQuestion = new LinkedHashMap<>();
		for (ScoreRule rule : projectEnd.parseScoreRules(projectEnd.ruleTextPane.getText(), projectEnd.standardAnswerCount())) {
	...
	private void renderStudentPreview(QuestionScoring.ScaleScoreReport report) {
		...
		List<QuestionScoring.ScaleStudentScore> scores = new ArrayList<>(report.studentScores());
		...
		for (QuestionScoring.ScaleStudentScore score : scores) {
	...
	public QuestionScoring.Config config() {
		return config;
	}

	public QuestionScoring.ScaleScoreReport report() {
		return report;
	}
}