...
public final class QuestionScoring {
	...
	public QuestionScoring(String[] standardAnswers,
	...
	private List<StudentAnswers> parseStudents() {
		...
		for (Map.Entry<String, String> entry : recognizedAnswers.entrySet()) {
			...
			students.add(new StudentAnswers(examineeId, answers, new Student(examineeId, correctFlags)));
		}
		return students;
	}
	...
	private boolean isAbsentInSection(StudentAnswers student, Section section) {
	...
	private double scoreSection(StudentAnswers student, Section section, double[] questionScores) {
		double score = 0.0;
		for (int idx : section.questionIndices()) {
	...
	private record StudentAnswers(String examineeId, String[] rawAnswers, Student student) {
	}
