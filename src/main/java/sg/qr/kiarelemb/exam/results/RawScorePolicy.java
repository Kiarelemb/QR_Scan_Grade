 ...
 * @className RawScorePolicy
 * @description TODO
 * @create 2026/6/18 18:31
 */
record RawScorePolicy(int lineNumber, String questionType, List<Integer> questionNumbers,
					  BigDecimal totalScore) {
	public static RawScorePolicy parse(String line, int lineNumber, int questionCount) {
		String[] parts = line.split("[ \\t]+");
		if (parts.length != 2 && parts.length != 3) {
			throw new ResultsPanel.ScoreRuleException(lineNumber, "第 " + lineNumber + " 行计分规则格式错误，应为：题型 题号范围 总分。");
		}
		...
		if (parts.length == 3) {
			try {
				...
				throw new ResultsPanel.ScoreRuleException(lineNumber, "第 " + lineNumber + " 行总分不是有效数字。");
			}
			if (totalScore.compareTo(BigDecimal.ZERO) < 0) {
				throw new ResultsPanel.ScoreRuleException(lineNumber, "第 " + lineNumber + " 行总分不能为负数。");
			}
		}
	...
	private static List<Integer> parseQuestionRange(String rangeText, int lineNumber, int questionCount) {
		...
		for (String range : ranges) {
			...
			if (part.contains("-")) {
				...
				if (bounds.length != 2) {
					throw new ResultsPanel.ScoreRuleException(lineNumber, "第 " + lineNumber + " 行题号范围格式错误。");
				}
			...
			if (start > end) {
				throw new ResultsPanel.ScoreRuleException(lineNumber, "第 " + lineNumber + " 行题号范围起点不能大于终点。");
			}
			if (start < 1 || end > questionCount) {
				throw new ResultsPanel.ScoreRuleException(lineNumber, "第 " + lineNumber + " 行题号超出范围，当前共有 " + questionCount + " 题。");
			}
	...
	private static int parseQuestionNumber(String value, int lineNumber) {
		try {
			...
			throw new ResultsPanel.ScoreRuleException(lineNumber, "第 " + lineNumber + " 行题号不是有效整数。");
		}
	}
}
